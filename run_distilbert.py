...
class BERT(nn.Module):
    def __init__ (self):
        ...
        self.out = nn.Linear(19200, 1)
        self.softmax = nn.Softmax(dim=0)
        
    def forward(self, ids, mask, token_type_ids):
        outputs = self.bert(ids, attention_mask = mask)
        bert_outputs = self.bert_drop(outputs.last_hidden_state)
        bert_outputs = torch.flatten(bert_outputs, start_dim=1)
        output = self.out(bert_outputs)
