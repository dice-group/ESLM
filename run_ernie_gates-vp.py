...
class ErnieGAT(nn.Module):
    ...
    def __init__(self, pretrained_model='nghuyong/ernie-2.0-en', nb_class=1):
        ...
        self.tokenizer = AutoTokenizer.from_pretrained(pretrained_model)
        self.bert_model = AutoModel.from_pretrained(pretrained_model)
        self.feat_dim = 768#list(self.bert_model.modules())[-2].out_features
        self.classifier = nn.Linear(self.feat_dim, nb_class)
    ...
    def forward(self, adj, input_ids, attention_mask, token_type_ids):
        ...
        features = outputs[0][:, 0]#outputs.pooler_output
        ...
        if mode == "train":
            for topk in config["topk"]:
                ...
                for fold in range(5):
                    ...
                    optimizer = torch.optim.AdamW(optimizer_grouped_parameters, lr=5e-2, eps=1e-8)
