...
model_bert = BertModel.from_pretrained(bert_version)
    ...
    if word_emb == "bert":
        for obj in obj_literal_list:
            ...
            with torch.no_grad():
                ...
                embeds = model_bert(**encodings)
                embeds = embeds[0]
                embeds = embeds[:, 0, :]
                #print(embeds[0])
                #print(embeds.shape)
                arrays_obj_literal_list.append(embeds[0].detach().numpy())
        obj_tensor = torch.tensor(arrays_obj_literal_list).unsqueeze(1)
